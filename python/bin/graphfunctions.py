...
def return_binstatistic(df, binned_column, stat_column, number_of_bins, folder, vrange=[0,1],SE=False, cutq=True, show = True, include_frame = True, test=""):
    ...
    filename = "../data/04/" + folder + "/" + err_strs + bin_strs + hotfixb + str(number_of_bins) + frame_str + test
    #h.single_stat_scatter(centres,means,all_lower,all_upper,title, binned_column, stat_column,filename, show, plot_error)
    h.mat_scat(centres,means,stdb,std,title,binned_column,stat_column,filename, show, plot_error, bins, vlinebool)
    plt.show()
...
column_plotted = 5
folder_path = "../data/test"
all_binstatistic(df, [list(df.columns)[10]], 25,"Summed_15")
'''
...
for cut in True, False:
    a = df[df['Phase Type'] == 'No Possession Change'].copy(deep = True)
   return_binstatistic(a, "Change in D Ball Goal", "Summed attacker space within 100m", 10, "Test", vrange=[0.05,0.95], cutq=cut, show = True)
   '''
